...
public class ProblemSolutions {
    ...
    private void mergeDivisbleByKFirst(int arr[], int k, int left, int mid, int right) {
        ...
        for (int j = 0; j < n2; j++) {
            if (arr[mid + 1 + j] % k == 0) {
                tmp[indx++] = arr[mid + 1 + j];
            }
        }

        int[] nonDivL = new int[n1];
        int[] nonDivR = new int[n2];
        int leftPtr = 0;
        int rightPtr = 0;

        for (int i = 0; i < n1; i++) {
            if (arr[left + i] % k != 0) {
                nonDivL[leftPtr++] = arr[left + i];
            }
        }

        for (int j = 0; j < n2; j++) {
            if (arr[mid + 1 + j] % k != 0) {
                nonDivR[rightPtr++] = arr[mid + 1 + j];
            }
        }
        ...
        while (i < leftPtr && j < rightPtr) {
            tmp[indx++] = nonDivL[i] <= nonDivR[j] ? nonDivL[i++] : nonDivR[j++];
        }
